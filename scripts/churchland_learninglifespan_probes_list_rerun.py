...
updated: group probes by the target(planned) position and add group name
...
the output dataframe includes:
    ...
    # priority scores, from 3 (high priority) to 0 (low priority)
        ...
        0: all the rest
    # planned position (x, y) if exist
    # target_location_name if exist
"""
...
merged_df['target_location_name'].fillna('Others', inplace=True)
#%save results 
merged_df.to_csv(os.path.join(datapath, "churchland_learninglifespan_project_info_probes_rerun.csv"), index=False) 
