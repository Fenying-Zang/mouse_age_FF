...
#%% load pids we use in the formal analysis
pid_eid_age_info = pd.read_parquet(os.path.join(datapath,'pid_eid_age_info.pqt'))
pids_filtered = pid_eid_age_info['pid']

#%% only save those one which is included in our formal analysis
pid2use=[]
for p in pids_filtered:
    if p in list(bwm_df['pid']):
        pid2use.append(p)

# % check version info for a list of pids 
info_version_df = pd.DataFrame({'pid':pid2use})
#%%
for index, row in info_version_df.iterrows():
    pid = row['pid']
    sl = SpikeSortingLoader(pid=pid, one=one, atlas=ba)
    info = one.alyx.rest('datasets', 'list', session=sl.eid, name='spikes.times.npy', collection=sl.collection)[0]
    
    info_version_df.loc[index,'eid']=sl.eid
    info_version_df.loc[index,'session']=info['session']
    info_version_df.loc[index,'revision']=info['revision']
    info_version_df.loc[index,'version']=info['version'] #2.35.2; 
    info_version_df.loc[index,'auto_datetime']=info['auto_datetime'] #2.35.2; 
    # print(version)

#%%save the results
bwm_df.to_excel(os.path.join(datapath, "check_BWM_ss_version.xlsx")) 

#%% try single pid here: you can check more in 'info'
sl = SpikeSortingLoader(pid=pid2use[5], one=one, atlas=ba)
info = one.alyx.rest('datasets', 'list', session=sl.eid, name='spikes.times.npy', collection=sl.collection)[0]
version = info['version'] #2.35.2; 
print(version)
